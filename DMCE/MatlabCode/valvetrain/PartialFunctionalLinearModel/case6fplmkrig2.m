...
for i=1:size(yrols,2)
   
   [mu_est,sigma2_est,ga_est,step] = krig(xk,yrols(:,i));
   mul=[mul,mu_est];
   sigma2l = [sigma2l,sigma2_est];
   gal = [gal,ga_est];
end;

 
fid=fopen('gaolskirg.m','w');
fprintf(fid,'%7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f %7.4f\n', gal);
fclose(fid)

 
fid=fopen('sigma2olskirg.m','w');
fprintf(fid,'%7.4f\n', sigma2l);
fclose(fid)

fid=fopen('muolskirg.m','w');
fprintf(fid,'%7.4f\n', mul);
fclose(fid)
