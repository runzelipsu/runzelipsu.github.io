...
ell0=kriglkhd(x,y,ga0)-n*sum( lambda^2 - (abs(ga0) - lambda).^2.*(abs(ga0) < lambda));
...
while (delta > eps) & (step<100)
   ...
   p1 = n*lambda*p10.*ga0;
	p2 = n*lambda*diag(p10.*ga0)*2;
   
   if step <=20
      theta0 = theta0 + 1*pinv(q2 + p2,epsr)*(q1 - p1);
   else
   ...
   ell1=kriglkhd(x,y,ga0)-n*sum( lambda^2 - (abs(ga0) - lambda).^2.*(abs(ga0) < lambda));
