...
ell0=kriglkhd(x,y,ga0)-n*lambda*sum( abs(ga0) );
...
while (delta > eps) & (step<100)
   ...
   p1 = n*lambda*aga;
	p2 = n*lambda*diag(aga)*1;
   
   if step <=20
      theta0 = theta0 + 1*pinv(q2 + p2)*(q1 - p1);
   else
      theta0 = theta0 + 0.5*pinv(q2 + p2)*(q1 - p1);
   end;
   
      
   ga0 =exp(theta0);
   
   ell1=kriglkhd(x,y,ga0)-n*lambda*sum( abs(ga0) );
