...
for k=1:K
   ...
   yhat0(k,1) =  mu(2) + R*beta0(:,2);
end;

f0 = mean(yhat0);
D0 = mean(yhat0.^2)- (mean(yhat0))^2;
...
for d=1:6 
    ...
    for l=1:N
      ...
      yhat1(d,l) = mean(temp1)-f0;
   end;
   D(d) = sum(yhat1(d,:).^2)*rx(d)/100;
end;

subplot(321)
a =x(:,1);
plot(a,y,'.',x0*rx(1)+minx(1),yhat1(1,:)+f0,'-')   
xlabel('Clearance')
ylabel('Noise')

subplot(322)
a =x(:,2);
plot(a,y,'.',x0*rx(2)+minx(2),yhat1(2,:)+f0,'-')   
xlabel('Pressure')
ylabel('Noise')

subplot(323)
a =x(:,3);
plot(a,y,'.',x0*rx(3)+minx(3),yhat1(3,:)+f0,'-')   
xlabel('Skirt Length')
ylabel('Noise')
...
subplot(324)
a =x(:,4);
plot(a,y,'.',x0*rx(4)+minx(4),yhat1(4,:)+f0,'-')   
...
subplot(325)
a =x(:,5);
plot(a,y,'.',x0*rx(5)+minx(5),yhat1(5,:)+f0,'-')   
...
subplot(326)
a =x(:,6);
plot(a,y,'.',x0*rx(6)+minx(6),yhat1(6,:)+f0,'-')   
xlabel('Pin Offset')
ylabel('Noise')
